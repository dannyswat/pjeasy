...
export function useGetReleaseCandidateItems(projectId: number, releaseId?: number | null, enabled = true) {
  const { data, isLoading, isError, error, refetch } = useQuery<ReleaseCandidateItemResponse[]>({
    queryKey: ['releaseCandidateItems', projectId, releaseId ?? null],
    queryFn: async () => {
